...
import pers.fancy.tools.scheduler.ScheduleTask;
import pers.fancy.tools.utils.Collections3;
import pers.fancy.tools.utils.JboxUtils;
import pers.fancy.tools.utils.ProxyTargetUtils;
...
public class ExecutorMonitor implements ScheduleTask, ExecutorLoggerInner {
    ...
    public void schedule() {
        ...
        List<Entry<String, ExecutorService>> jBoxExecutors = ExecutorManager.executors.entrySet()
                .stream()
                .filter(entry -> entry.getKey().startsWith("com.github.tools"))
                .sorted((e1, e2) -> e2.getKey().length() - e1.getKey().length())
                .collect(Collectors.toList());

        List<Entry<String, ExecutorService>> bizExecutors = ExecutorManager.executors.entrySet()
                .stream()
                .filter(entry -> !entry.getKey().startsWith("com.github.tools"))
                .sorted((e1, e2) -> e2.getKey().length() - e1.getKey().length())
                .collect(Collectors.toList());
        ...
        for (Entry<String, ExecutorService> entry : jBoxExecutors) {
            logGroupDetail(entry.getKey(), entry.getValue(), maxGroupSize);
        }

        for (Entry<String, ExecutorService> entry : bizExecutors) {
            logGroupDetail(entry.getKey(), entry.getValue(), maxGroupSize);
        }
    }
    ...
    private Object[] getFlightRecorder(String group) {
        ExecutorManager.FlightRecorder recorder = ExecutorManager.recorders.getOrDefault(group, new ExecutorManager.FlightRecorder());
